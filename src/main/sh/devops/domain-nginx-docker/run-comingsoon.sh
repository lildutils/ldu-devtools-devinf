...
_init() {
    task=comingsoon-start

    _healthCheck

    networkName=$(node -p -e "require('${PWD}/DEV-INF/configs.json').frontend.networkName")

    containerName=$(node -p -e "require('${PWD}/DEV-INF/configs.json').frontend.comingsoon.containerName")

    redirectPortNumber=$(node -p -e "require('${PWD}/DEV-INF/configs.json').frontend.comingsoon.redirectPortNumber")
    dockerPortNumber=$(node -p -e "require('${PWD}/DEV-INF/configs.json').frontend.comingsoon.dockerPortNumber")

    defaultRegion=$(node -p -e "require('${PWD}/DEV-INF/configs.json').frontend.comingsoon.defaultRegion")

    version=1.0.0-COMINGSOON
