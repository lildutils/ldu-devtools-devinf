...
_init() {
    ...
    network=$(node -p -e "require('./configs.json').network")
    containerName=$(node -p -e "require('./configs.json').containerName")
    defaultRegion=$(node -p -e "require('./configs.json').defaultRegion")
